...
1. 开发时生成 source map 方便调试

2. 发布到 npm 前删除所有  `.map`  文件

3. 或者把 source map 上传到内部错误追踪系统（如 Sentry），而不是打包进发布物
...
1. **检查发布物内容**：用  `npm pack`  预览将要发布的文件列表

2. **配置 .npmignore**：明确排除  `*.map`、`*.ts`、`src/`  等不应发布的内容

3. **使用构建检查工具**：在 CI/CD 流程中加入发布前检查

4. **分离 source map**：如果需要 source map，上传到私有服务，不要打包进公开产品
...
// 读取 source map
const sourceMap = JSON.parse(fs.readFileSync('cli.js.map', 'utf8'))
